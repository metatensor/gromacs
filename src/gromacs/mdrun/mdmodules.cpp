...
#include "gromacs/applied_forces/metatensor/metatensor_mdmodule.h"
...
public:
    Impl() :
        ...
        metatensor_(MetatensorModuleInfo::create())
    {
    }
    ...
    void makeModuleOptions(Options* options) const
    {
        ...
        metatensor_->mdpOptionProvider()->initMdpOptions(&appliedForcesOptions);
    ...
    std::unique_ptr<IMDModule>      metatensor_;
...
void MDModules::initMdpTransform(IKeyValueTreeTransformRules* rules)
{
    ...
    impl_->metatensor_->mdpOptionProvider()->initMdpTransform(appliedForcesScope.rules());
}
...
void MDModules::buildMdpOutput(KeyValueTreeObjectBuilder* builder)
{
    ...
    impl_->metatensor_->mdpOptionProvider()->buildMdpOutput(builder);
}
...
ForceProviders* MDModules::initForceProviders()
{
    ...
    impl_->metatensor_->initForceProviders(impl_->forceProviders_.get());
...
void MDModules::subscribeToPreProcessingNotifications()
{
    ...
    impl_->metatensor_->subscribeToPreProcessingNotifications(&impl_->notifiers_);
}
